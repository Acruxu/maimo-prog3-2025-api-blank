...
const findAllProducts = async (req, res) => {
  try {
    const products = await Product.find().select("_id name team driver price stock colors description sizes details image categories ");
...
const findOneProduct = async (req, res) => {
    const {id} = req.params
    try {
        const product = await Product.findOne({_id: id}).select("_id name team driver price stock colors description sizes details image categories ")
